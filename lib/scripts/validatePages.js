...
module.exports = function validatePages({
  ...
  return fetch(url.origin)
    .then(async () => {
      ...
      const errorNumberTab = languages.map(async (lang) => {
        const outputPath = `${outputDir}/${lang}`;
        createDirectory(outputPath);
        const allPromises = Object.keys(pagePaths).map(
          async (path) =>
